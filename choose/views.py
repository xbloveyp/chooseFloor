...
@csrf_exempt
def index(request):
    randomNum, building_data, high_floor = simulation(request)
    building_data_view = []
...
def setFloor(request):
    ...
    for i in range(len(building_data)):
        ...
        building_item_map["building"] = i + 1
        building_data_view.append(building_item_map)
    return render(request, 'chooseFloorSub.html', {'building_data': list(building_data),'building_data_view':building_data_view, 'randomNum': str(randomNum)})
